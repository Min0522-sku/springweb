...
public class ThreadPoolConfig {
    // **** 서버 안정화 ****
    @Bean // 해당 함수를 등록, 클래스 자체 등록하는것과 다르게 함수/필드 단위로 별도 빈 등록
    public Executor taskExecutor(){ // Executor 반환 타입은  import java.util.concurrent.Executor;
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(2); // 기본(최소) 실행 스레드 개수 설정
        executor.setMaxPoolSize(3); // 최대 실행 스레드 개수 설정
        // 만일 최대 개수 이상의 요청 및 스레드 사용시 이후 요청은 대기상태
        executor.setQueueCapacity(3); // 대기열 최대 개수
        // queue 큐 : 대기열(웨이팅줄)
