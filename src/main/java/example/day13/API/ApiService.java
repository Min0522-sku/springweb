...
public class ApiService {
    ...
    public Map<String, Object> test1(){
        ...
        uri += "&pageNo=1"; // 요청 매개변수 1, 페이지 번호
        uri += "&perPage=67"; // 요청 매개변수 2, 페이지당 보여줄 자료 개수
        return webClient.get()
                .uri(uri) // 요청할 api 주소 넣어준다. url vs uri(매개변수 포함)
                .retrieve() // 반환/통신/응답 결과
                .bodyToMono(Map.class) // 반환 값을 자바 타입으로 변환, 반환 타입이 JSON 이면 MAP으로 받는다.
                .block(); // 동기(처리가 끝날때 까지 대기상태) 방식으로 결과 반환
    }
    // [1] 국립중앙의료원_전국 약국 정보 조회 서비스 XML
    // ** 서비스키를 주소상에 포함하지 않고 HTTP header에 포함
    ...
    public Map<String, Object> test2(){
        ...
        String response = webClient.get()
                ...
                // HTTP 헤더: HTTP통신할때 부가정보를 포함하는 정보
                // 주로 인증 관련된 정보들을 포함하는 경우 / API키, 로그인 상태 등
        ...
        try {
            XmlMapper xmlMapper = new XmlMapper(); // xml 매퍼 객체 생성
                //xmlMapper.readValue(반환할값, 변환할타입.class);
            Map<String, Object> map = xmlMapper.readValue(response, Map.class);
            return map;
        } catch (JsonProcessingException e) {System.out.println("e = " + e);}
       return null;
    }
}
    /*
    ...
    종류
        ...
        2. 활용 : 무료/유료 판단
            ...
            3) 사기업
                ...
                등등
    반환타입 : JSON / XML
    스프링에서 외부 HTTP 요청, 예) 자바 내에서 서로다른 프로젝트 or 서로다른 서비스  끼리의 통신
